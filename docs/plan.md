...
- **focus の識別子は `terminal_id` に確定**。`herdr agent focus <terminal_id>` を使う（本家踏襲。target は
  terminal id / agent 名 / ラベルを受け付けるが、一意で安定な terminal_id を採用）。
  - 実際の focus 動作は他 pane を奪うため未実行。**実機での focus 成功確認は 0004/0005 の受入条件**とし、
    それまでは「コマンド仕様の確定」に留める（progress.md に明記）。
...
- **表示順（決定的にする。JSON 順に依存しない）**:
  ...
  - ワークスペース内のエージェントは `terminal_id` の昇順（安定・決定的なら他キーでも可だが terminal_id を既定）。
...
- **focus の実行タイミングと失敗処理**:
  - Bubble Tea の model は「選択された target(terminal_id)」を返すだけにし、**program 終了・端末復元後**に
    main が `herdr agent focus <terminal_id>` を実行する（raw mode 中に外部コマンドを叩かない。本家踏襲）。
...
> 解決済み（§3.1 / §3.2 へ移動）: 識別子＝terminal_id / git_context 方式 / 種別・タイトル・git の
> フォールバック / 表示順・0 件 workspace / ラベル上限・入力状態 / focus タイミング。

- `internal/` のパッケージ分割粒度（0004 で model/render 純ロジックと統合を分けるかを含む）。
- Bubble Tea でのポップアップ描画と herdr の pane placement(`popup`) の兼ね合い（0004/0005 で実機確認）。
- `herdr agent focus <terminal_id>` の実機での成功確認（0004/0005 の受入条件）。
